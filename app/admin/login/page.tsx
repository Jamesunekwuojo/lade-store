  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        // Full page transition to ensure the session cookie is applied to middleware
...
export default function AdminLoginPage() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[var(--background)]">
      <Suspense fallback={<div className="text-xs tracking-widest uppercase text-[var(--muted-foreground)]">Loading Portal...</div>}>
        <LoginForm />
      </Suspense>
    </div>
  );
